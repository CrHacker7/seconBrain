...
A ***while*** loop makes it possible to repetitively execute code based on a certain codition
...
> [!note] (at least one of the is 1) | 
> 0 0 0 0 1 1 1 1 = 15  
   0 0 0 1 0 1 1 0 = 22  
  \--------------
   0 0 0 1 1 1 1 1   = 31
   
> [!warning] (only one of the bits has to be 1) ^ 
> 0 0 0 0 1 1 1 1   = 15  
   0 0 0 1 0 1 1 0   = 22  
  \--------------  
   0 0 0 1 1 0 0 1     = 25
...
> [!TIP] Bit shifting
> **\>>** BIt Shifting Right (double it down)
> **\<<** Bit Shifting Left (double it up)

> [!note] (move one digit to the right) \>>
> **print(22 >> 1)**
> 0 0 0 1 0 1 1 0 = 22
> 0 0 0 0 1 0 1 1 = 11

> [!success] (move one digit to the Left) \<<
> **print(22 << 1)**
> 0 0 0 1 0 1 1 0 = 22
> 0 0 1 0 1 1 0 0 = 44
...
**print(22 * 2)** --> **print(22 << 1)**
**print(22 * 4)** --> **print(22 << 2)**
- Perfoming a binary right shift is the same as an integer division by **two** and shifting it by two is the same as an integer division by **four**.
- A binary left shifted by one is the same as multiplying it by two and shifting it by two bits would be the same as multiplying it by four.
1. Logical operators ***and not*** and ***or*** return boolean values based on the values that we pass.
2. Bitwise operators ***& |*** and ***~*** allow us to manipulate single bits of data, and return **0** or **1** based on the bits that are passed or the bits that correspond to the integer that we pass (not use with floating point numbers).
3. Bit shifting can be done with the ***<<*** and ***>>*** operators which shifts the bits a certain amount to the right or to the left, and returns the integer that corresponds to the new set of bits

> [!tip] ~n = -n - 1
> A cualquier número positivo, ~n lo vuelve negativo y le suma 1 al valor.
> 
> Ejemplo: ~5 = -6 (es -5 - 1)
> 
> A cualquier número negativo, ~n lo vuelve positivo y le resta 1 al valor.
> 
> Ejemplo: ~(-5) = 4 (es 5 - 1)
...
Desplazamos 2 posiciones a la izquierda: 00001100 = 12 decimal
8 & 12
8 en binario: 00001000

12 en binario: 00001100

AND bit a bit: 00001000 (solo el bit en posición 3 está activo en ambos)
